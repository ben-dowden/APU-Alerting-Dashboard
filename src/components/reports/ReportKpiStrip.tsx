...
export function ReportKpiStrip({ report, mode }: ReportKpiStripProps) {
  ...
  const fourthHelper = mode === "ops" ? "Largest reason for selected metric" : "If the top reason is reduced by 50%";
  ...
  return (
    <section className="metric-grid metric-grid--reports">
      ...
      <MetricCard label="Estimated cost" value={`$${report.totalCostAud}`} helper={`$${report.costPerBurnHour} per burn hour`} tone="red" icon={<Banknote size={22} />} />
      <MetricCard label="Avoidable cost" value={`$${report.avoidableCostAud}`} helper={`${report.avoidableBurnHours} avoidable hours`} tone="green" icon={<Fuel size={22} />} />
